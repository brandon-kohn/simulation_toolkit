...
cmake -DCMAKE_TOOLCHAIN_FILE=%VCPKG_ROOT%/scripts/buildsystems/vcpkg.cmake -H. -Bcmake.build.%CONFIG% -DBUILD_TESTS=1 -DCMAKE_BUILD_TYPE=%CONFIG% -DCMAKE_INSTALL_PREFIX="cmake.install" -G%CMAKE_GENERATOR% %ARCHITECTURE%
if ERRORLEVEL 1 goto error
...
ctest --output-on-failure -C %CONFIG% %3 %4 %5 %6 %7 %8 %9
if ERRORLEVEL 1 goto error
