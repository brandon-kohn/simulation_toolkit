...
# Build the release
cmake -H. -Bcmake.build -DBUILD_TESTS=1 -DCMAKE_BUILD_TYPE=$"CONFIG" -DCMAKE_INSTALL_PREFIX="cmake.install" -G"$CMAKE_GENERATOR" || error_exit "CMake Release Build failed. Aborting."

# Install it
cmake --build "cmake.build" --config $"CONFIG" || error_exit "CMake Install failled. Aborting"
