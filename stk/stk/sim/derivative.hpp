...
namespace stk {
	...
	struct derivative_constant
	{
		...
		result_type operator()( const Args&... a ) const
		{
			return v;
		}
	...
	struct pow_fun
	{
		...
		{
			using type = T;
			BOOST_FORCEINLINE static type apply( const T& t )
			{
				using std::pow;
				return pow( t, Exp );
			}
		...
		struct result<This( boost::units::quantity<Units, T> )>
		{
			...
			BOOST_FORCEINLINE static type apply( const boost::units::quantity<Units, T>& t )
			{
	...
	struct sqrt_fun
	{
		...
		struct result<This( boost::units::quantity<Units, T> )>
		{
			...
			BOOST_FORCEINLINE static type apply( const boost::units::quantity<Units, T>& t )
			{
				...
				return boost::units::root<2>( abs( t ) );
			}
		};
	...
	namespace derivative_detail {
		...
		struct make_optimized_binary
		{
			...
			BOOST_FORCEINLINE
				typename result<make_optimized_binary<Tag>( A0, A1 )>::type const
	...
	namespace proto {
		template <typename Tag>
		struct is_callable<stk::derivative_detail::make_optimized_binary<Tag>>
			: mpl::true_
		{};
