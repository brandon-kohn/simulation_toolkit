#ifndef STK_CONTAINER_MULTI_INDEX_ORDERED_UNIQUE_INDEX_HPP
#define STK_CONTAINER_MULTI_INDEX_ORDERED_UNIQUE_INDEX_HPP
#pragma once
...
namespace stk { 
    ...
    class ordered_unique_index_iterator 
        : public boost::iterator_adaptor
          <
              ordered_unique_index_iterator< Collection, Iterator, Value >// Derived
            , Iterator                  // Base
            , Value                     // Value
    ...
    {
        struct enabler {};
        typedef typename ordered_unique_index_iterator::iterator_adaptor_ super;
        friend class boost::iterator_core_access;
        typedef Iterator iterator;    

    public:

        ordered_unique_index_iterator(Collection* pC = 0)
            : super( iterator() )
            , m_pCollection(pC)
        {}

        explicit ordered_unique_index_iterator( Collection* c, iterator it )
            : super(it)
            , m_pCollection(c)
        {}

        template <typename OtherCollection, typename OtherIterator, typename OtherValue>
        ordered_unique_index_iterator
            (
                ordered_unique_index_iterator<OtherCollection, OtherIterator, OtherValue> const& other
    ...
    struct ordered_unique_index 
    {
        ...
        private:
            typedef boost::container::flat_set<std::size_t, detail::indirect_compare_adaptor<base_container_type, Compare> > container_type;
        ...
        public:
            ...
            typedef ordered_unique_index_iterator<base_container_type, typename container_type::iterator, Value>       iterator;
            typedef ordered_unique_index_iterator<const base_container_type, typename container_type::const_iterator, const Value> const_iterator;
            ...
            const_iterator find(const CompatibleKey& v) const
            {
                //! Need an indirect way to compare values for std::lower_bound                
                BOOST_AUTO(it, std::lower_bound(begin(), end(), v, m_cmp));
                if( it != end() && !m_cmp(v, *it) ) 
                    return it;
                else
                    return end();
            }
            ...
            template<typename CompatibleKey>
            iterator find(const CompatibleKey& v) 
            {
                //! Need an indirect way to compare values for std::lower_bound                
                BOOST_AUTO(it, std::lower_bound(begin(), end(), v, m_cmp));
                if( it != end() && !m_cmp(v, *it) ) 
            ...
            iterator find(const Value& v)
            {
                ...
                BOOST_AUTO(it, std::lower_bound(m_index.begin(), m_index.end(), (std::numeric_limits<std::size_t>::max)(), cmp));
                if( it != m_index.end() && !m_cmp(v, get_base_container()[*it]) ) 
                    return iterator(&get_base_container(), it);
                else
                    return end();
            }
            ...
            const_iterator find(const Value& v) const
            {
                ...
                BOOST_AUTO(it, std::lower_bound(m_index.begin(), m_index.end(), (std::numeric_limits<std::size_t>::max)(), cmp));
                if( it != m_index.end() && !m_cmp(v, get_base_container()[*it]) ) 
                    return const_iterator(&get_base_container(), it);
                else
                    return end();
            }
        ...
        protected:
            ...
            {   
                detail::indirect_compare_value_adaptor<base_container_type, Value, Compare> cmp(get_base_container(), v, m_cmp);
                auto it = std::lower_bound(m_index.begin(), m_index.end(), (std::numeric_limits<std::size_t>::max)(), cmp);
                if( it == m_index.end() || cmp((std::numeric_limits<std::size_t>::max)(), *it) ) 
                {                    
                    std::size_t index( get_base_container().size() );
                    std::size_t res = super::insert_impl(v);
                    if( res == index )
                        m_index.insert(it, index);                
                    return res;
                }
                else
                {
            ...
            {
                typename container_type::iterator it = m_index.find(i);
                if( it != m_index.end() )
                    m_index.erase(it);
...
#endif // STK_CONTAINER_MULTI_INDEX_ORDERED_UNIQUE_INDEX_HPP
