	...
	private:
		...
		void expand()
		{
			...
			if (m_isExpanding.compare_exchange_strong(isExpanding, true))
			{
				...
				{
					auto growSize = growth_policy().growth_factor( m_blocks.back().size() );
					m_blocks.emplace_back( growSize );
					auto it = m_blocks.back().begin();
					auto gen = [&]()
					{ return &*it++; };
					m_q.generate_bulk( gen, growSize );
				}
				m_isExpanding.store( false );
				m_itemsLoaded.notify_all();
			}
			else if( m_q.size_approx() == 0 )
			{
				auto lk = std::unique_lock<std::mutex>{ m_loadMutex };
				m_itemsLoaded.wait( lk, [this](){ return !m_isExpanding.load( std::memory_order_relaxed ) || m_q.size_approx() > 0; } );
			}
