...
namespace thread {
	...
	struct cv_idle_wait_policy
	{
		...
		{
			auto lk = typename Traits::template unique_lock<mutex_type>{ ctx.mtx };
			ctx.cv.wait( lk, std::forward<Pred>( pred ) );
		}

		static void notify_one( context& ctx ) noexcept { ctx.cv.notify_one(); }
		static void notify_all( context& ctx ) noexcept { ctx.cv.notify_all(); }
	};

	template <typename QTraits = locked_queue_traits, typename Traits = boost_thread_traits, typename Tag = default_threadpool_tag, typename WaitPolicy = cv_idle_wait_policy<Traits>>
	class work_stealing_thread_pool : boost::noncopyable
	{
		...
		using wait_policy = WaitPolicy;
		using wait_context = typename wait_policy::context;
		...
		void worker_thread( std::uint32_t tIndex, bool bindToProcs, std::false_type ) //! Q has queue_info
		{
			...
			while( true )
			{
				if( hasTask )
				{
					...
					if( !m_stopThread[tIndex].load( std::memory_order_relaxed ) ) [[likely]]
					{
						spincount = 0;
				...
				else
				{
					...
					//! Policy decides whether to keep spinning/backing off
					if( wait_policy::on_idle_iteration( spincount ) )
					{
						if( !m_stopThread[tIndex].load( std::memory_order_relaxed ) ) [[likely]]
							hasTask = poll( queueInfo, tIndex, lastStolenIndex, task );
						else
					...
					else
					{
						...
						wait_policy::wait( m_wait,
							[&queueInfo, tIndex, &lastStolenIndex, &hasTask, &task, this]()
							{
								return ( hasTask = poll( queueInfo, tIndex, lastStolenIndex, task ) )
									|| m_stopThread[tIndex].load( std::memory_order_relaxed )
									|| m_done.load( std::memory_order_relaxed );
							} );
			...
			while( true )
			{
				...
				else
				{
					...
					else
					{
						...
						wait_policy::wait( m_wait,
							[tIndex, &lastStolenIndex, &hasTask, &task, this]()
							{
								return ( hasTask = poll( tIndex, lastStolenIndex, task ) )
									|| m_stopThread[tIndex].load( std::memory_order_relaxed )
									|| m_done.load( std::memory_order_relaxed );
							} );
	...
	public:
		...
		{
			auto pred = [this]() BOOST_NOEXCEPT
			{ return !has_outstanding_tasks(); };
			wait_for( pred );
		}
	...
	private:
		...
		void parallel_for_impl( Range&& range, TaskFn&& task, std::size_t nthreads, std::size_t npartitions, std::false_type ) BOOST_NOEXCEPT
		{
			...
			std::size_t njobs = 0;
			partition_work( range, npartitions, [nthreads, &njobs, &fs, &task, this]( iterator_t from, iterator_t to ) -> void
				{
		...
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		std::vector<padded_atomic_bool> m_stopThread;
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		std::atomic<bool> m_done { false };
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		queue_type m_poolQ;
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		std::vector<padded_queue<queue_type>> m_localQs;
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		std::atomic<std::uint32_t> m_active { 0 };
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		std::atomic<std::uint32_t> m_nThreads { 0 };
		BOOST_ALIGNMENT( STK_CACHE_LINE_SIZE )
		scalable_task_counter m_nTasksOutstanding;

		[[no_unique_address]] wait_context m_wait{};
