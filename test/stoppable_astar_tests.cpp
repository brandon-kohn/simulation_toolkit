...
TEST( StoppableAstarTestSuite, stoppable_astar_search_ThreeNodeGraphSearchingFromV1ToV2_VisitorProperlyTerminatesSearch )
{
	...
	stoppable_astar_search( g, start, distance_heuristic<Graph>( g, goal ), predecessor_map( &preds[0] ).distance_map( &costs[0] ).weight_map( boost::get( &EdgeProperties::weight, g ) ).visitor( astar_goal_visitor<Vertex>( goal, visitorTerminated ) ).distance_inf( std::numeric_limits<::units::length>::infinity() ).distance_zero( 0.0 * boost::units::si::meters ) );
...
TEST( StoppableAstarTestSuite, stoppable_astar_search_tree_ThreeNodeGraphSearchingFromV1ToV2_VisitorProperlyTerminatesSearch )
{
	...
	stoppable_astar_search_tree( g, start, distance_heuristic<Graph>( g, goal ), predecessor_map( &preds[0] ).distance_map( &costs[0] ).weight_map( boost::get( &EdgeProperties::weight, g ) ).visitor( astar_goal_visitor<Vertex>( goal, visitorTerminated ) ).distance_inf( std::numeric_limits<::units::length>::infinity() ).distance_zero( 0.0 * boost::units::si::meters ) );
...
namespace {
	...
	protected:
		...
		std::vector<std::uint8_t>          edge_mask;
	};
